...
################################################################################
if {[::legacy::find -design design:multiplier_param_N8] ne ""} {
  puts "** A design with the same name is already loaded. It will be removed. **"
  delete_obj design:multiplier_param_N8
}
...
# Design
################################################################################
read_netlist -top multiplier_param_N8 layout/logical_multiplier_param_N8.v.gz
read_metric -id current layout/logical_multiplier_param_N8.metrics.json

phys::read_script layout/logical_multiplier_param_N8.g.gz
...
# slack
set _slk_ [::legacy::get_attribute slack design:multiplier_param_N8]
if {[regexp {^-?[0-9.]+$} $_slk_]} {
  set _slk_ [format %.1f $_slk_]
}
if {$_slk_ != "440.8"} {
   mesg_send [::legacy::find -message /messages/PHYS/PHYS-92] "golden slack: 440.8,  current slack: $_slk_"
}
unset _slk_
# multi-mode slack
# tns
set _tns_ [::legacy::get_attribute tns design:multiplier_param_N8]
if {[regexp {^-?[0-9.]+$} $_tns_]} {
...
unset _tns_
# cell area
set _cell_area_ [::legacy::get_attribute cell_area design:multiplier_param_N8]
if {[regexp {^-?[0-9.]+$} $_cell_area_]} {
  set _cell_area_ [format %.0f $_cell_area_]
}
if {$_cell_area_ != "965"} {
   mesg_send [::legacy::find -message /messages/PHYS/PHYS-92] "golden cell area: 965,  current cell area: $_cell_area_"
}
unset _cell_area_
# net area
set _net_area_ [::legacy::get_attribute net_area design:multiplier_param_N8]
if {[regexp {^-?[0-9.]+$} $_net_area_]} {
  set _net_area_ [format %.0f $_net_area_]
}
if {$_net_area_ != "366"} {
   mesg_send [::legacy::find -message /messages/PHYS/PHYS-92] "golden net area: 366,  current net area: $_net_area_"
}
unset _net_area_
