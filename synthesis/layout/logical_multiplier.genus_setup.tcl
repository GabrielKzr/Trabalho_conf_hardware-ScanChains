...
################################################################################
if {[::legacy::find -design design:multiplier] ne ""} {
  puts "** A design with the same name is already loaded. It will be removed. **"
  delete_obj design:multiplier
}
...
# Design
################################################################################
read_netlist -top multiplier layout/logical_multiplier.v.gz
read_metric -id current layout/logical_multiplier.metrics.json

phys::read_script layout/logical_multiplier.g.gz
...
# slack
set _slk_ [::legacy::get_attribute slack design:multiplier]
if {[regexp {^-?[0-9.]+$} $_slk_]} {
...
# multi-mode slack
# tns
set _tns_ [::legacy::get_attribute tns design:multiplier]
if {[regexp {^-?[0-9.]+$} $_tns_]} {
...
unset _tns_
# cell area
set _cell_area_ [::legacy::get_attribute cell_area design:multiplier]
if {[regexp {^-?[0-9.]+$} $_cell_area_]} {
  set _cell_area_ [format %.0f $_cell_area_]
}
if {$_cell_area_ != "1967"} {
   mesg_send [::legacy::find -message /messages/PHYS/PHYS-92] "golden cell area: 1967,  current cell area: $_cell_area_"
}
unset _cell_area_
# net area
set _net_area_ [::legacy::get_attribute net_area design:multiplier]
